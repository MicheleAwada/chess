...
import {
	movePiece,
	allPieceMoves as getAllPieceMoves,
} from "./logic/standard piece logic";
...
function GetSquare({
	...
	stateBoard: [number[][], any];
	row: number;
	col: number;
	id: number;
	setAllPossibleMoves: any;
	stateSelectedPiece: [number[], any];
	possibleMoves: number[][];
	isWhitePlayer: boolean;
	isWhitesPlay: boolean;
	setIsWhitesPlay: any;
}) {
	// const realRowCol: [number, number] = [row,col];
	// const [realRow, realCol] = oldRowCol;
	if (!isWhitesPlay) {
	...
	const is_selected_square =
		selectedPiece && arraysAreEqual([row, col], selectedPiece);
	let is_possible_square = possibleMoves.some((subArray) => {
		return arraysAreEqual(subArray, rowCol);
	});

	// const isNumberedSquare = col===0
	// const squareNumberLabel = 7-row+1

	// const letters = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h'];
	// const isLetteredSquare = row===7
	// const squareLetterLabel = letters[col]
	...
	return (
		<div
			onClick={() => {
				if (is_possible_square) {
					...
					const robot_board_move = robot_move_board(
						person_board_move,
						!isWhitesPlay
					);
					...
					const allPossibleMoves = getAllPieceMoves(
						robot_board_move,
						isWhitesPlay
					);
					setAllPossibleMoves(allPossibleMoves);
				} else if (
					isWhitePlayer === isWhitesPlay &&
					isWhitesPlay === isWhitePiece
				) {
		...
		>
			{!is_empty_square && (
				<div className="square_size abs-tl piece_image_padding">
					<img
						src={pieceInfo[pieceName].image[+isWhitePiece]}
						alt={pieceName}
						className="parent-full-size piece_image"
					/>
				</div>
			)}
			{is_selected_square && (
				<div className="selected-square-shade square_size abs-tl"></div>
			)}
			{(is_possible_square &&
				(is_empty_square ? (
					...
					<div className="square_size possible_square_eat_padding abs-tl">
				...
				return (
					<div className={`row`} key={i}>
						{row.map((col: number, j: number) => {
							return (
								<GetSquare
									setAllPossibleMoves={setAllPossibleMoves}
									isWhitePlayer={isWhitePlayer}
									isWhitesPlay={isWhitesPlay}
									setIsWhitesPlay={setIsWhitesPlay}
									stateBoard={stateBoard}
									row={i}
									col={j}
									id={col}
									key={j}
									stateSelectedPiece={stateSelectedPiece}
...
function findPossibleMoveIndex(
	allPossibleMoves: [[number, number], [number, number][]][],
...
function App() {
	const stateBoard: [number[][], any] = useState(
		createMatrix(initialBoardPiece)
	);
	const [board, setBoard]: [number[][], any] = stateBoard;
	const [isWhitePlayer] = useState(Math.random() < 0.5);
	const [isWhitesPlay, setIsWhitesPlay]: [boolean, any] = useState(true);
	useEffect(() => {
		if (!isWhitePlayer) {
			const robot_board_move = robot_move_board(board, isWhitesPlay);
			setBoard(robot_board_move);
			setIsWhitesPlay(!isWhitesPlay);
		}
	}, []);
	const stateSelectedPiece = useState([]);
	const [selectedPiece]: [any, any] = stateSelectedPiece;
	const [possibleMoves, setPossibleMoves]: [[number, number][] | [], any] =
		useState([]);
	const [allPossibleMoves, setAllPossibleMoves]: [
		[[number, number], [number, number][]][],
		any
	] = useState(getAllPieceMoves(board, isWhitePlayer));
	useEffect(() => {
