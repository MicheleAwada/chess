import { getInfoFromId, pieceInfo } from  "../App"
...
function pieceInTheWay(board:number[][], row:number, col:number, isWhitesMove: boolean):number {
    ...
    if (isWhitesMove ? pieceColorIsWhite : !pieceColorIsWhite) {  // same color piece cant eat  
        return 2
    }
    return 1
}

function laserPieceMove(board:number[][], row:number, col:number, moves_direction: [number, number][]): [number, number][] {
    const possible_moves: [number, number][] = [];
    const pieceId = board[row][col];
    const isWhitePiece = isWhiteFromId(pieceId)
    for (let d=0; d<4; d++) {
        for (let i=1; i<8; i++) {
            const move_pos: [number, number] = [moves_direction[d][0]*i, moves_direction[d][1]*i]
            ...
            const pieceInTheWayOutput = pieceInTheWay(board, new_pos_row, new_pos_col, isWhitePiece)
            if (pieceInTheWayOutput===0 || pieceInTheWayOutput===1) { //if another colors piece you can eat it but not further than that
...
function bishopPieceMove(board:number[][], row:number, col:number): [number, number][] {
    const moves: [number, number][] = [[1,1], [-1,1], [1,-1], [-1,-1]] // oblique path
    return laserPieceMove(board, row, col, moves)
}
function rookPieceMove(board:number[][], row:number, col:number): [number, number][] {
    const moves: [number, number][] = [[1,0], [0,1], [-1,0], [0,-1]] //linear path
    return laserPieceMove(board, row, col, moves)
}
function queenPieceMove(board:number[][], row:number, col:number): [number, number][] {
    return [...rookPieceMove(board, row, col), ...bishopPieceMove(board, row, col)]
}

function knightPieceMove(board:number[][], row:number, col:number): [number, number][] {
    const moves: [number, number][] = [[2,1], [2,-1], [-2,1], [-2,-1], [1,2], [1,-2], [-1,2], [-1,-2]]
    const possible_moves: [number, number][] = [];
    const pieceId = board[row][col];
    const isWhitePiece = isWhiteFromId(pieceId)
    moves.forEach((move_pos: [number,number], index: number) => {
        ...
        const pieceInTheWayOutput = pieceInTheWay(board, to_move_row, to_move_col, isWhitePiece)
...
function pawnPieceMove(board:number[][], row:number, col:number): [number, number][] {
    const possible_moves = []
    let moves: [number, number][] = [[1, 0], [1, 1], [1, -1], [2, 0]]
    ...
    if (board[move_forward_pos_row][move_forward_pos_col] === 0) { //pawn can move forward but not obstructed
        possible_moves.push(move_forward_pos)
    }
    const double_move_forward_pos = add_rowCols(moves[3], [row, col])
    if ((isWhitesPlay ? row===8-1-1 : row===1) && board[move_forward_pos_row][move_forward_pos_col] === 0 && board[double_move_forward_pos[0]][double_move_forward_pos[1]] === 0) { // has not moved yet, allows for double move on first move and no piece infront and no piece in the way
        possible_moves.push(double_move_forward_pos)
    }

    function add_if_can_eat_forward(board:number[][], rowCol:[number, number], move: [number, number]) {
        const new_pos = add_rowCols(rowCol, move)
        const pieceInTheWayOutput = pieceInTheWay(board, new_pos[0], new_pos[1], isWhitesPlay)
        if (pieceInTheWayOutput === 1 && pos_exists_in_board(new_pos[0], new_pos[1])) {
            possible_moves.push(new_pos)
        }
...
function possiblePieceMoves(board: number[][], row:number, col:number): [number,number][] {
    ...
    let check_for_full_check = !isChecked
    check_for_full_check = true

    if (pieceInfo["king"].id === abs_id) {
        check_for_full_check = true
        for (let i:number=-1; i<2; i++) {
            for (let j:number=-1; j<2; j++) {
                const king_to_move_pos: [number, number] = add_rowCols([i,j], [row,col]);
                const [king_to_move_pos_row, king_to_move_pos_col] = king_to_move_pos;
                if ((i===0 && j===0) || !pos_exists_in_board(king_to_move_pos_row, king_to_move_pos_col) || pieceInTheWay(board, king_to_move_pos_row, king_to_move_pos_col, isWhitesPlay) === 2 ) {
                    continue
                }
...
export function allPieceMoves(board:number[][], color:boolean) {
    ...
    board.forEach((rows:number[], i:number) => {
        rows.forEach((col:number, j:number) => {
            ...
            const piecePossibleMovesWithOrigin: [[number, number], [number, number][]] = [[i,j], piecePossibleMoves] //[from, to]
...
export function movePiece(board:number[][], rowColFrom: number[], rowColTo: number[]) {
...
function check_check(board:number[][], checkForWhiteKing: boolean):boolean {
    ...
    const check_each_piece: [string[], (board:number[][], row:number, col:number) => [number, number][]][] = [
    ...
    for (const [key, value] of check_each_piece) {
        isChecked = value(board, king_pos[0], king_pos[1]).some((rowCol:[number, number]) => {
            return key.some((pieceType:string) => {
                if (pieceInfo[pieceType].id * enemy_color_multiplier === board[rowCol[0]][rowCol[1]]) {
                    return true;
                }
                return false;
                })
